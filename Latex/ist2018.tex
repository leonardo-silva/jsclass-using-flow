...
\title{Retrieving Class Dependencies using a Type Inferencer: An Empirical Assessment}
...
\begin{abstract}
Context: Accurately identifying dependencies between software components is an essential activity when maintaining and evolving software applications. It is also known that developers often use classes to tackle the complexity of legacy JavaScript code, \emph{i.e.}, code implemented in JavaScript versions that do not provide syntactical support to classes. In this scenario, identifying class dependencies is specially challenging due to the lack of types in the source code. 
%In this scenario, identifying associations and other dependencies between classes is specially challenging due to the lack of types in the source code. 

Objective: This paper investigates the use of type inference to identify relationships between classes in legacy JavaScript code. 

Method: We demonstrate how to use Flow, a static type-checker for JavaScript, to infer types that correspond to class references in systems transpiled from TypeScript to JavaScript. We manually analyzed and categorized all false negatives identified in our study. Finally, we successfully apply our approach in a set of vanilla JavaScript systems. 

Results: Our results show that precision reaches 100\% in the evaluated systems. Flow's recall ranges from 85\% to 96\% for associations, 64\% to 99\% for \aspas{uses} dependencies, and, if we consider both, from 80\% to 98\%. 

Conclusions: The overall results suggest that using a type inferencer is a reliable approach to identify dependencies between software components in JavaScript.
...
\begin{center}
	\noindent \fbox{\parbox{0.98\textwidth}{
			{\em Summary: } By eliminating and expanding the code of \mcode{require} statements precision improves to 100\% in the analyzed systems. The values of recall range from 80\% to 98\% for all dependencies.}}
\end{center}
...
The authors would like to thank CNPq, CAPES and FAPEMIG. 
